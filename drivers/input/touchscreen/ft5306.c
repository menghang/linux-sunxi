...
#define TS_RESET_LOW_PERIOD		(5)
#define TS_INITIAL_HIGH_PERIOD		(30)
#define TS_WAKEUP_LOW_PERIOD	(50)
#define TS_WAKEUP_HIGH_PERIOD	(20)
#define TS_POWER_LOW_PERIOD		(20)
#define TS_POWER_HIGH_PERIOD	(100)
...
static int gpio_power_enable = 1;
...
static int ctp_init_platform_resource(void)
{
	...
	if(!gpio_power_hdle) {
		pr_warning("%s: tp_power_port request gpio fail!\n", __func__);
		gpio_power_enable = 0;
	}
...
/**
 * ctp_powerup - function
 *
 */
static void ctp_power(void)
{
	if(1 == gpio_power_enable){  
		//pr_info("%s. \n", __func__);
		//if(EGPIO_SUCCESS != gpio_write_one_pin_value(gpio_wakeup_hdle, 0, "ctp_power_port")){
		//	pr_info("%s: err when operate gpio. \n", __func__);
		//}
		//mdelay(TS_POWER_LOW_PERIOD);
		if(EGPIO_SUCCESS != gpio_write_one_pin_value(gpio_wakeup_hdle, 1, "ctp_power_port")){
			pr_info("%s: err when operate gpio. \n", __func__);
		}
		mdelay(TS_POWER_HIGH_PERIOD);
...
static struct ctp_platform_ops2 ctp_ops = {
...
static int __init ft5x_ts_init(void)
{
	...
	pr_info("===========================%s=====================\n", __func__);
	pr_info("FT5306 driver for P85 ported by MoonShadow\n");
...
MODULE_AUTHOR("<wenfs@Focaltech-systems.com> moonshadow@HIT");
